...
const sortText = document.querySelector("#sort_button");
...
function start() {
	...
	//eventlisteners på filtrerings-knapper
	const filterButtons = document.querySelectorAll("button[data-cakes]");
	filterButtons.forEach(button => button.addEventListener("click", filterCakes));
	//eventlisteners på alle sorteringsknapper
...
function sortCakesTime() {
	...
	showCakes(sort);
	sortText.textContent = "Tid (stigende)";
}
...
function sortCakesTimeUp() {
	...
	//kalder showcakes med den sorterede variabel (så den har samme filter)
	showCakes(sort);
	sortText.textContent = "Tid (faldende)";
}
